...
export const Reportes = () => {
  ...
  if (loading) return <div className="p-20 text-center text-[var(--color-acento)] font-black uppercase text-[0.8em] tracking-[0.3em]">Cargando Analíticas...</div>;

  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20" style={{ fontSize: 'var(--fontSize)' }}>
      ...
      <div className="bg-[var(--color-componente)] p-10 rounded-[3rem] shadow-2xl border border-[var(--color-borde)] flex flex-col lg:flex-row justify-between items-center gap-8">
        <div>
          ...
          <p className="text-[0.6em] font-bold opacity-40 uppercase tracking-[0.4em] mt-3 text-[var(--color-texto-componente)]">Control de ingresos y capital humano</p>
        </div>
        <div className="flex gap-4 bg-[var(--color-secundario)] p-4 rounded-[2rem] border border-[var(--color-borde)] shadow-inner">
          <input type="date" value={fechaInicio} onChange={(e)=>setFechaInicio(e.target.value)} className="bg-[var(--color-fondo)] rounded-xl text-[0.7em] font-bold text-[var(--color-texto-componente)] p-3 outline-none border border-[var(--color-borde)] focus:border-[var(--color-acento)]" />
          <input type="date" value={fechaFin} onChange={(e)=>setFechaFin(e.target.value)} className="bg-[var(--color-fondo)] rounded-xl text-[0.7em] font-bold text-[var(--color-texto-componente)] p-3 outline-none border border-[var(--color-borde)] focus:border-[var(--color-acento)]" />
        </div>
      </div>

      {/* GRÁFICA DINÁMICA */}
      <div className="bg-[var(--color-componente)] p-12 rounded-[4rem] shadow-2xl border border-[var(--color-borde)] relative">
        <h3 className="text-[0.7em] font-bold text-[var(--color-acento)] uppercase tracking-[0.4em] mb-16">Rendimiento Diario (MXN)</h3>
        <div className="relative h-[400px] w-full flex items-end justify-between gap-4 border-b border-[var(--color-borde)] pb-2">
          {dataGrafica.map((item, idx) => (
            <div key={idx} className="flex-grow flex flex-col items-center group relative z-10">
              ...
              <span className="text-[0.6em] font-bold opacity-40 uppercase mt-6 tracking-tighter text-[var(--color-texto-componente)]">{item.fecha}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-[var(--color-componente)] p-10 rounded-[3rem] shadow-2xl border border-[var(--color-borde)]">
        <h3 className="text-[0.7em] font-bold text-[var(--color-acento)] uppercase tracking-[0.4em] mb-8">⚠️ Alerta de Inventario Crítico</h3>
          ...
          {stockAlerta.length > 0 ? stockAlerta.map((item, i) => (
            <div key={i} className="bg-[var(--color-secundario)] p-6 rounded-[2rem] border border-[var(--color-borde)] flex flex-col justify-between hover:border-[var(--color-acento)] transition-all">
              <div>
                ...
                <p className="text-[0.5em] font-bold text-[var(--color-acento)] uppercase tracking-widest opacity-60">Última: {item.ultima_fecha}</p>
              </div>
              <div className="mt-6 flex items-end justify-between text-[var(--color-texto-componente)]">
                <div>
                  <p className="text-[0.5em] font-black opacity-30 uppercase">Stock</p>
                  <p className="text-3xl font-black text-[var(--color-acento)] leading-none">{item.stock_actual}</p>
                </div>
                <div className="text-right">
                  <p className="text-[0.5em] font-black opacity-30 uppercase">Mínimo</p>
            ...
            <div className="col-span-full p-10 bg-[var(--color-secundario)] rounded-[2rem] border border-emerald-900/30 text-center">
               <p className="text-[0.7em] font-bold text-emerald-500 uppercase tracking-widest">✅ Inventario en niveles óptimos</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
        <div className="bg-[var(--color-componente)] p-10 rounded-[3rem] shadow-2xl border border-[var(--color-borde)]">
          <h3 className="text-[0.7em] font-bold text-[var(--color-acento)] uppercase tracking-[0.4em] mb-8">Flujo de Caja</h3>
          <div className="space-y-4">
            {Object.entries(corteCaja).map(([llave, total]) => (
              <div key={llave} className="flex justify-between items-center bg-[var(--color-secundario)] p-5 rounded-2xl border border-[var(--color-borde)]">
                <span className="text-[0.6em] font-black opacity-40 text-[var(--color-texto-componente)] uppercase tracking-widest">{llave}</span>
        ...
        <div className="xl:col-span-2 bg-[var(--color-componente)] p-10 rounded-[3rem] shadow-2xl border border-[var(--color-borde)]">
          <h3 className="text-[0.7em] font-bold text-[var(--color-acento)] uppercase tracking-[0.4em] mb-8">Liquidación de Personal</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="text-[0.6em] font-black opacity-30 uppercase border-b border-[var(--color-borde)] text-[var(--color-texto-componente)]">
              ...
              <tbody className="divide-y divide-[var(--color-borde)]">
                {nominaMaestra.map((est, i) => (
                  <tr key={i} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="py-4 text-center font-bold text-emerald-500">${est.comision.toFixed(2)}</td>
                    <td className="py-4 text-center font-bold text-rose-500">${(est.usoInterno + est.vales).toFixed(2)}</td>
      ...
      <div className="bg-[var(--color-componente)] p-12 rounded-[4rem] shadow-2xl border border-[var(--color-borde)] border-l-8 border-l-[var(--color-acento)]">
        <h3 className="text-[0.7em] font-bold text-[var(--color-acento)] uppercase tracking-[0.4em] mb-10">Deudas Pendientes</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {resumenDeudas.length > 0 ? resumenDeudas.map((res, i) => (
            <div key={i} className="bg-[var(--color-secundario)] p-8 rounded-[2.5rem] border border-[var(--color-borde)] flex justify-between items-center">
              <div><h4 className="font-serif italic text-2xl text-[var(--color-texto-componente)]">{res.nombre}</h4><p className="text-[0.6em] font-bold opacity-30 uppercase mt-1 tracking-widest text-[var(--color-texto-componente)]">Saldo</p></div>
              <div className="text-right"><span className="font-bold text-3xl text-rose-500">${res.total.toFixed(2)}</span></div>
            </div>
          )) : <p className="opacity-30 italic text-[0.6em] uppercase tracking-widest text-[var(--color-texto-componente)]">No hay deudas activas.</p>}
        </div>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
        <div className="bg-[var(--color-componente)] p-12 rounded-[4rem] shadow-2xl border border-[var(--color-borde)]">
          <h3 className="text-[0.7em] font-bold text-[var(--color-acento)] uppercase tracking-[0.4em] mb-8">Top 5 Servicios</h3>
          <table className="w-full text-left text-[var(--color-texto-componente)]">
            <thead>
              <tr className="text-[0.6em] font-black opacity-30 uppercase border-b border-[var(--color-borde)]"><th className="pb-4">Nombre</th><th className="pb-4">Vendidos</th><th className="pb-4 text-right text-[var(--color-acento)]">Total</th></tr>
            </thead>
            <tbody>
              {obtenerTop5('servicio').map((s, i) => (
                <tr key={i} className="border-b border-[var(--color-borde)] hover:bg-white/5 transition-all"><td className="py-4 font-serif italic text-xl">{s.nombre}</td><td className="py-4 text-xs">{s.cantidad}</td><td className="py-4 text-right font-serif text-xl text-[var(--color-acento)]">${s.total.toFixed(2)}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
        ...
        <div className="bg-[var(--color-componente)] p-12 rounded-[4rem] shadow-2xl border border-[var(--color-borde)]">
          <h3 className="text-[0.7em] font-bold text-[var(--color-acento)] uppercase tracking-[0.4em] mb-8">Top 5 Productos</h3>
          <table className="w-full text-left text-[var(--color-texto-componente)]">
            <thead>
              <tr className="text-[0.6em] font-black opacity-30 uppercase border-b border-[var(--color-borde)]"><th className="pb-4">Nombre</th><th className="pb-4">Vendidos</th><th className="pb-4 text-right text-[var(--color-acento)]">Total</th></tr>
            </thead>
            <tbody>
              {obtenerTop5('producto').map((p, i) => (
                <tr key={i} className="border-b border-[var(--color-borde)] hover:bg-white/5 transition-all"><td className="py-4 font-serif italic text-xl">{p.nombre}</td><td className="py-4 text-xs">{p.cantidad}</td><td className="py-4 text-right font-serif text-xl text-[var(--color-acento)]">${p.total.toFixed(2)}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
